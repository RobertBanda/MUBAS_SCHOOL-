...
<style>
  ...
  header {
    background: #1976d2;
        ...
        $custRes = $conn->query("SELECT Customer_ID, Name FROM customers ORDER BY Name");
        ...
        document.getElementById('customer_id').addEventListener('change', function() {
          ...
          document.getElementById('new_customer_name').style.display = show ? 'block' : 'none';
          document.getElementById('new_customer_name').required = show;
        });
        </script>
        </form>
        <?php
              ...
              echo "<tr>
                <form method='post'>
                  ...
                  <td><select name='edit_customer_id' required style='width:100%'>";
                    $custRes2 = $conn->query("SELECT Customer_ID, Name FROM customers ORDER BY Name");
...
<nav>
  ...
  <button data-tab="meters" onclick="showTab('meters')">Meters</button>
  <button data-tab="connections" onclick="showTab('connections')">Connections</button>
  <button data-tab="bills" onclick="showTab('bills')">Bills</button>
  <button data-tab="meterreadings" onclick="showTab('meterreadings')">Meter Readings</button>
...
document.getElementById('units_consumed').addEventListener('input', function() {
  ...
  <div id="dashboard" class="tab active">
    ...
    <?php
      ...
      $totalRevenue = $conn->query("SELECT SUM(Amount_Paid) as total FROM payments")->fetch_assoc()['total'];
      $pendingComplaints = $conn->query("SELECT COUNT(*) as total FROM complaints WHERE Status!='Resolved'")->fetch_assoc()['total'];
    ?>
    <div class="dashboard-cards">
      ...
      <div class="dashboard-card">Revenue<br><span>MK<?= $totalRevenue ?></span></div>
  ...
  <form method="post" style="margin-bottom:2rem; background:#f9fbff; padding:1.5rem; border-radius:10px; box-shadow:0 1px 4px rgba(0,0,0,0.04);">
        <div style="display:flex; flex-wrap:wrap; gap:1rem; align-items:flex-end;">
          <div style="flex:1; min-width:120px;">
            ...
            <select id="bill_id" name="bill_id" required style="width:100%;padding:0.5rem; border-radius:5px; border:1px solid #d0d7e6;">
              <option value="">--Select--</option>
              <?php
              $billRes = $conn->query("SELECT Bill_ID FROM bills ORDER BY Bill_ID DESC");
      ...
      <table>
        ...
        <?php
        $res = $conn->query("SELECT * FROM payments ORDER BY Payment_ID DESC");
        $edit_payment_id = isset($_GET['edit_payment']) ? intval($_GET['edit_payment']) : 0;
        while($row = $res->fetch_assoc()){
  ...
  if (
    $_SERVER['REQUEST_METHOD'] === 'POST' &&
    isset($_POST['name'], $_POST['address'], $_POST['phone'], $_POST['email'], $_POST['category']) &&
    !isset($_POST['update_customer_id'])
  ) {
  ...
  $sql = "INSERT INTO customers (Name, Address, Phone, Email, Category) VALUES ('$name', '$address', '$phone', '$email', '$category')";
      if ($conn->query($sql)) {
        echo '<div style="color:green;margin-bottom:1rem;">Customer added successfully.</div>';
      } else {
        echo '<div style="color:red;margin-bottom:1rem;">Error: ' . $conn->error . '</div>';
      }
    } else {
...
?>
    <table>
      <tr><th>ID</th><th>Name</th><th>Address</th><th>Phone</th><th>Email</th><th>Category</th><th>Action</th></tr>
      <?php
  ...
  <div id="meters" class="tab">
    ...
    if (
      $_SERVER['REQUEST_METHOD'] === 'POST' &&
      isset($_POST['meter_number'], $_POST['customer_id'], $_POST['installation_date'], $_POST['status'], $_POST['last_service_date'])
    ) {
      $meter_number = $conn->real_escape_string(trim($_POST['meter_number']));
      $customer_id = intval($_POST['customer_id']);
      $installation_date = $conn->real_escape_string(trim($_POST['installation_date']));
      $status = $conn->real_escape_string(trim($_POST['status']));
      $last_service_date = $conn->real_escape_string(trim($_POST['last_service_date']));
      if ($meter_number && $customer_id && $installation_date && $status && $last_service_date) {
        // Check for duplicate meter number
        $dupRes = $conn->query("SELECT Meter_ID FROM meters WHERE Meter_Number = '$meter_number'");
        if ($dupRes && $dupRes->num_rows > 0) {
          echo '<div style="color:red;margin-bottom:1rem;">Error: Meter number already exists. Please use a unique meter number.</div>';
        } else {
          $sql = "INSERT INTO meters (Customer_ID, Meter_Number, Installation_Date, Status, Last_Service_Date) VALUES ($customer_id, '$meter_number', '$installation_date', '$status', '$last_service_date')";
          if ($conn->query($sql)) {
            echo '<div style="color:green;margin-bottom:1rem;">Meter added successfully.</div>';
          } else {
            echo '<div style="color:red;margin-bottom:1rem;">Error: ' . $conn->error . '</div>';
          }
        }
    ...
    <table>
      ...
      <?php
  $res = $conn->query("SELECT M.*, C.Name as Customer_Name FROM meters M JOIN customers C ON M.Customer_ID=C.Customer_ID ORDER BY M.Meter_ID DESC");
